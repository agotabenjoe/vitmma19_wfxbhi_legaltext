...
# Assuming model.py is in the same directory or accessible via path
import sys
sys.path.append(os.path.dirname(os.path.abspath(__file__))) 
from model import LSTMClassifier, TextDataset

# ==========================================
# PART 1: CONSENSUS ANALYSIS LOGIC
# ==========================================

def analyze_consensus_quality(consensus_folder, output_dir):
    """
    Reads raw JSONs to calculate Agreement Rates and Disagreements.
    ...
    # Updated path logic to be robust
    json_files = glob.glob(os.path.join(consensus_folder, "*.json"))
    if not json_files:
        logger.warning(f"⚠️ No consensus files found in {consensus_folder}. Skipping analysis.")
        return

    stats = {
        'total_tasks': 0,
        'perfect_alignment': 0, # Everyone agreed
        'disagreement': 0,      # Majority existed but not unanimous
        'ties': 0,              # No majority (discarded data)
        'avg_agreement_rates': []
    }

    logger.info(f"   Processing {len(json_files)} annotator files...")

    # Aggregating all annotations by Text
    ...
    for text, labels in text_map.items():
        stats['total_tasks'] += 1
        n_votes = len(labels)
        if n_votes < 2: continue # Skip single annotations if any
    ...
    logger.info(f"   ⚠️ Disagreement (Majority Exists): {stats['disagreement']} ({(stats['disagreement']/stats['total_tasks']*100):.1f}%)")
...
# ==========================================
# PART 2: MODEL EVALUATION LOGIC
# ==========================================

def extract_features(text):
    """Simple feature extraction for the baseline model."""
...
def main():
    # --- PATHS ---
    # Assuming script is in work/src (or similar) and data is in work/data
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Go up one level to get out of 'src' if needed, or adjust relative path
    # If your script is in work/src/eval.py, then '..' takes you to 'work/'
    ...
    # Set Consensus folder location to data/legaltextdecoder/consensus
    ...
    if os.path.exists(consensus_raw_path):
        ...
        logger.warning(f"⚠️ Consensus raw folder not found at {consensus_raw_path}. Skipping data quality analysis.")

    # 2. LOAD DATASET (Pre-Split)
    logger.info("\n" + "="*40)
    ...
    test_ds = dataset['test'] # Consensus Data (Ties dropped)
    ...
    # FIX: Ensure labels are 0-4 (if raw data is 1-5)
    # This subtracts 1 from the raw label to make it 0-indexed for Sklearn
    ...
    # Print example predictions for baseline
    ...
    if os.path.exists(weights_path):
        ...
        # Prepare Test Loader
        # IMPORTANT: Ensure your TextDataset handles the label conversion internally or do it here
        ...
        # DEBUG: Check first batch to ensure labels are correct
        first_batch = True
        
        with torch.no_grad():
            for x, y in test_loader:
                if first_batch:
                    # Print raw labels from loader to confirm they are 0-4
        ...
        # Print example predictions for LSTM
